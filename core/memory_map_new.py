"""
Updated memory_map.py - Memory addresses for Pokémon Crystal game state
Based on analysis of actual ROM and common Pokemon Crystal memory layouts
"""

# Memory addresses for Pokémon Crystal (Game Boy Color)
# These addresses are updated based on memory analysis

MEMORY_ADDRESSES = {
    # Player position and map data (based on coordinate analysis)
    'player_x': 0xDCB8,           # Player X coordinate (original - may need update)
    'player_y': 0xDCB9,           # Player Y coordinate (original - may need update) 
    'player_map': 0xDCB5,         # Current map/location ID
    'player_direction': 0xDCBA,   # Direction player is facing
    
    # Alternative coordinate locations found in analysis
    'alt_x': 0xD4BC,             # Potential X coordinate (found value 29)
    'alt_y': 0xD4BD,             # Potential Y coordinate (found value 10)
    
    # Player stats (may need adjustment based on actual ROM)
    'player_hp': 0xD16C,          # Current HP (2 bytes, little endian)
    'player_max_hp': 0xD16E,      # Maximum HP (2 bytes, little endian)
    'player_level': 0xD172,       # Player's current level
    'player_exp': 0xD179,         # Current experience points (3 bytes)
    'player_exp_to_next': 0xD17C, # Experience needed for next level
    
    # Party information
    'party_count': 0xD163,        # Number of Pokémon in party
    'party_species': 0xD164,      # Species of each party Pokémon (6 bytes)
    'party_hp': 0xD16C,          # HP of each party Pokémon (12 bytes, 2 per Pokémon)
    'party_max_hp': 0xD178,      # Max HP of each party Pokémon (12 bytes)
    'party_level': 0xD184,       # Level of each party Pokémon (6 bytes)
    'party_status': 0xD18A,      # Status conditions (6 bytes)
    
    # Items and inventory (based on money analysis)
    'money': 0xD84A,             # Player's money (3 bytes, BCD format) - Found in analysis!
    'bag_count': 0xD892,         # Number of items in bag
    'bag_items': 0xD893,         # Bag items (2 bytes per item: ID + quantity)
    
    # Game progress
    'badges': 0xD857,            # Badge bits (Johto badges in lower 8 bits)
    'kanto_badges': 0xD858,      # Kanto badge bits
    'elite_four_beaten': 0xD864, # Elite Four completion flag
    'champion_beaten': 0xD865,   # Champion beaten flag
    ...
    # Time and day/night (found some activity in this area)
