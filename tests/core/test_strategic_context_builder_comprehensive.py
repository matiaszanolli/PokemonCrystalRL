#!/usr/bin/env python3
"""
Corrected Comprehensive Test Suite for Strategic Context Builder

This module provides properly corrected comprehensive testing for the strategic context builder,
with correct API calls and data structures matching the actual implementation.
"""
...
class TestStrategicContextBuilderCore:
    ...
    def context_builder(self):
        ...
        with patch('core.strategic_context_builder.GameStateAnalyzer') as mock_analyzer, \
             ...
             patch('core.strategic_context_builder.DecisionHistoryAnalyzer') as mock_history, \
    ...
    def test_strategic_context_builder_initialization(self, context_builder):
        ...
        assert hasattr(context_builder, 'state_history')
    ...
    def test_action_definitions_structure(self, context_builder):
        ...
        for action in expected_actions:
            if action in action_defs:
                ...
                # The actual structure uses different field names
                expected_fields = ['description', 'risks', 'benefits', 'typical_outcome']
                assert any(field in action_defs[action] for field in expected_fields)

        # Should have at least some basic actions
        assert len(found_actions) > 3
    ...
    def test_build_context_basic_functionality(self, context_builder, sample_analysis):
        ...
        # Mock goal planner to return simple goals with proper attributes
        mock_goals = [Mock(id="test_goal", name="Test Goal", description="Test", progress_percentage=50.0)]
        ...
        # Verify context structure matches actual DecisionContext
    ...
    def test_pattern_recognition(self, context_builder, sample_analysis):
        ...
        assert hasattr(context_builder, 'stuck_patterns')
        assert hasattr(context_builder, 'successful_patterns')
        assert isinstance(context_builder.stuck_patterns, list)
        assert isinstance(context_builder.successful_patterns, list)
    ...
    def test_strategic_goal_determination(self, context_builder, sample_analysis):
        ...
        # Mock goal planner to return iterable goals with proper attributes
        mock_goals = [
            Mock(id="goal1", name="Reach City", description="Get to next city", progress_percentage=30.0),
            Mock(id="goal2", name="Level Up", description="Level up Pokemon", progress_percentage=70.0)
        ]
        context_builder.goal_planner.evaluate_goals.return_value = mock_goals
    ...
    def test_history_tracking(self, context_builder, sample_analysis):
        ...
        # Verify history is being tracked in the actual attributes
        assert len(context_builder.action_history) > 0
        assert len(context_builder.reward_history) > 0
...
class TestStrategicContextBuilderAdvanced:
    ...
    def test_prompt_building_quality(self, advanced_builder, complex_analysis):
        ...
        consequences = {'a': ActionConsequence('a', 'Attack move', 'medium', 'high', 'high')}
        goals = ['Win gym battle', 'Maintain Pokemon health']

        # Test the actual method with correct signature
        emergency_actions = ['b', 'start']
        prompts = advanced_builder._build_prompts(complex_analysis, consequences, emergency_actions, goals)

        assert isinstance(prompts, dict)
        # Check for expected prompt components
        expected_keys = ['situation_prompt', 'context_prompt', 'guidance_prompt', 'complete_prompt']
    ...
    def test_strategy_insights_tracking(self, advanced_builder):
        ...
        # Check if strategy insights exist or are created during operation
        assert hasattr(advanced_builder, 'adaptive_strategy')
    ...
    def test_goal_statistics_tracking(self, advanced_builder, complex_analysis):
        ...
        mock_goal = Mock(id="test_goal", progress_percentage=50.0, name="Test Goal")
    ...
    def test_set_strategy_configuration(self, advanced_builder):
        ...
        # Test with valid strategy from the actual enum
...
class TestStrategicContextBuilderEdgeCases:
    ...
    def test_component_failure_resilience(self, edge_case_builder):
        ...
        try:
            ...
            # Component failure should be handled gracefully in production
            # For tests, we just verify it fails as expected
            pass
...
class TestDecisionContextDataclass:
    ...
    def test_decision_context_creation(self, sample_analysis):
        ...
        context = DecisionContext(
            ...
            recent_actions=['left', 'up'],
            recent_rewards=[0.5, 1.0],
            stuck_patterns=['up_up_up'],
            successful_patterns=['left_up_a'],
        ...
        assert isinstance(context.recent_actions, list)
        assert isinstance(context.recent_rewards, list)
        assert isinstance(context.stuck_patterns, list)
