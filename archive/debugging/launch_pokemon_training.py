...
class PokemonTrainingLauncher:
    ...
    def skip_pokemon_crystal_intro(self, trainer, max_attempts=30):
        """
        Skip Pokemon Crystal intro sequence to reach controllable gameplay.
        
        Args:
            trainer: UnifiedPokemonTrainer instance
            max_attempts: Maximum button presses to attempt
            
        Returns:
            bool: True if gameplay was reached, False otherwise
        """
        print("🎮 Skipping Pokemon Crystal intro sequence...")
        
        for attempt in range(max_attempts):
            # Press START button to skip logos, enter menus, and advance cutscenes
            trainer.strategy_manager.execute_action(7)  # START button
            
            # Check game state every few attempts
            if attempt % 5 == 0:
                try:
                    screen = trainer.pyboy.screen.ndarray
                    variance = np.var(screen.astype(np.float32))
                    
                    print(f"   Attempt {attempt}: screen variance = {variance:.1f}")
                    
                    # Check if we've reached rich gameplay content
                    if variance > 7000:  # Overworld gameplay has high variance
                        print(f"✅ Reached Pokemon Crystal overworld gameplay!")
                        
                        # Test player movement to confirm controllable state
                    ...
                    if attempt == 0:
                        print(f"   ⚠️ Screen access error: {e}")
                    continue
        ...
        try:
            ...
            print(f"   Final screen variance: {final_variance:.1f}")
            
            if final_variance > 5000:
                print("✅ Pokemon Crystal intro skip successful")
                return True
            else:
                print("❌ Pokemon Crystal intro skip incomplete")
                return False
        except Exception as e:
            print(f"❌ Pokemon Crystal intro skip failed: {e}")
            return False
    ...
    def launch_training(self, config: TrainingConfig, web_port: int = 5000):
        ...
        try:
            ...
            if not self.skip_pokemon_crystal_intro(self.trainer):
