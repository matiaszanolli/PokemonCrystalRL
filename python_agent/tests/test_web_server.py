...
from trainer.web_server import TrainingWebServer, TrainingHandler
...
class TestTrainingWebServer(unittest.TestCase):
    ...
    @patch('trainer.web_server.socket.socket')
...
class TestTrainingHandler(unittest.TestCase):
    def setUp(self):
        ...
        with patch('trainer.web_server.BaseHTTPRequestHandler.__init__'):
    ...
    @patch('trainer.web_server.BaseHTTPRequestHandler.__init__')
    ...
    @patch('trainer.web_server.os.path.join')
    @patch('trainer.web_server.os.path.dirname')
    @patch('trainer.web_server.os.path.abspath')
    ...
    def test_serve_api_status(self):
        ...
        with patch('trainer.web_server.time.time', return_value=1234567950):
            with patch.object(self.handler, 'send_response') as mock_response, \
                 patch.object(self.handler, 'send_header') as mock_header, \
                 patch.object(self.handler, 'end_headers') as mock_end:
    ...
    @patch('trainer.web_server.PSUTIL_AVAILABLE', True)
    @patch('trainer.web_server.psutil')
    ...
    @patch('trainer.web_server.PSUTIL_AVAILABLE', False)
...
class TestWebServerIntegration(unittest.TestCase):
    ...
    def test_server_lifecycle(self, mock_socket):
        ...
        with patch('trainer.web_server.HTTPServer') as mock_http_server:
            mock_server_instance = Mock()
            mock_http_server.return_value = mock_server_instance
