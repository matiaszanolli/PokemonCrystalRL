...
class TrainerWebMonitorBridge:
        ...
        try:
            ...
            if not hasattr(self.web_monitor, 'training_session') or not self.web_monitor.training_session:
                ...
                class MockTrainingSession:
                    ...
                    def env(self):
                        # Mock environment for get_game_state
                        class MockEnv:
                            def get_game_state(self):
                                return {
                                    'player': {
                                        'x': getattr(self.trainer, '_player_x', 10),
                                        'y': getattr(self.trainer, '_player_y', 8), 
                                        'map': getattr(self.trainer, '_current_map', 1),
                                        'money': 0,
                                        'badges': 0
                                    },
                                    'party': []
    ...
    def get_bridge_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'screenshot_fps': 1.0 / self.screenshot_update_interval if self.screenshot_update_interval > 0 else 0,
